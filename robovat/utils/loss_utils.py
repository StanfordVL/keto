...
def l2_loss(targets,
    ...
    if clip_value is not None:
        assert isinstance(clip_value, float)
        loss = tf.maximum(loss, clip_value)
    return tf.losses.compute_weighted_loss(loss, weights, reduction=reduction)


def reconst_angle_loss(targets, outputs, weights=1.0,
                       reduction=Reduction.SUM_BY_NONZERO_WEIGHTS):
...
# def kl_divergence_gaussian(mean, stddev, weights=1.0):
#     loss = 0.5 * tf.reduce_sum(
#         (tf.square(mean) + tf.square(stddev) -
#          tf.log(1e-8 + tf.square(stddev)) - 1.0),
#         axis=-1)
#     return tf.losses.compute_weighted_loss(loss, weights)
