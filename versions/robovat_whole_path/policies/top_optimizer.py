...
class CEM(object):
    ...
    def __call__(self, inputs, samples, seed):
        ...
        def body(i, samples, scores):
            ...
            elites = tf.batch_gather(samples, ind)
            ...
            if self._debug:
                print_op = tf.print(
                    ...
                    'elite_scores: ', tf.batch_gather(scores, ind), '\n',
                )
        ...
        if samples.shape[0] > self._num_samples:
            ...
            samples = tf.batch_gather(samples, ind)
            scores = tf.batch_gather(scores, ind)

        _, final_samples, final_scores = tf.while_loop(
            cond=cond,
            body=body,
            loop_vars=[0, samples, scores])

        _, ind = tf.nn.top_k(final_scores, k=1)
        top_sample = tf.batch_gather(final_samples, ind)
        top_score = tf.batch_gather(final_scores, ind)
